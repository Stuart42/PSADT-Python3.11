...
Try {
	...
	[string]$appVersion = '3.11'
	[string]$appArch = 'X64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '11/09/2022'
	[string]$appScriptAuthor = 'Stuart Adams'
	##*===============================================
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'python,python_d,pythonw,pythonw_d' -CloseAppsCountdown 60

		## Show Progress Message (With a Message to Indicate the Application is Being Uninstalled)
        $InstalledPythonVersions = Get-InstalledApplication -Name 'Python 3'
		if ($InstalledPythonVersions.count -ge 1) {
			# Show-InstallationProgress -StatusMessage "Removing Any Existing Versions of Python 3 - Please Wait..."
			## Retrieve version and architecture of old install to remove
			$RemoveVersion, $RemoveArch = ($InstalledPythonVersions | Where-Object DisplayName -Like '*Executable*-bit)').DisplayName.Split(' ')[1, 3]
			$UninstallPath = 'not found'
			if ($RemoveArch -like '*64*') {
				# $UninstallPath = (Get-ChildItem $dirSupportFiles | Where-Object Name -like "*$RemoveVersion-*64*.exe").Name
				Write-Log -Message "Checking $dirSupportFiles for $RemoveVersion exe"
				$UninstallPath = (Get-ChildItem -Path $dirSupportFiles -Filter "*$RemoveVersion-*64*.exe" | Select-Object -First 1)
				Write-Log -Message "Uninstall path for 64-bit found; $($UninstallPath.FullName)"
			}
			else {
				$UninstallPath = (Get-ChildItem -Path $dirSupportFiles -Filter "*$RemoveVersion.exe" | Select-Object -First 1)
				Write-Log -Message "Uninstall path for 32-bit found; $($UninstallPath.FullName)"
			}
			if ($UninstallPath -eq 'not found'){
		...
		Write-Log -Message "Running installer $InstallerPath"
		Execute-Process -Path $InstallerPath.FullName -Parameters "/quiet unattend.xml" -WindowStyle Hidden
